...
set(_exported_dependencies "rclcpp;rclcpp_action;std_msgs;nav2_util;nav2_msgs;nav2_core;nav2_behavior_tree;nav_msgs;geometry_msgs;opennav_coverage_msgs;behaviortree_cpp")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to opennav_coverage_bt_DEFINITIONS, opennav_coverage_bt_INCLUDE_DIRS,
# opennav_coverage_bt_LIBRARIES, and opennav_coverage_bt_LINK_FLAGS.
# Additionally collect the direct dependency names in
# opennav_coverage_bt_DEPENDENCIES as well as the recursive dependency names
# in opennav_coverage_bt_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(opennav_coverage_bt_DEPENDENCIES ${_exported_dependencies})
  set(opennav_coverage_bt_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(opennav_coverage_bt_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(opennav_coverage_bt_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(opennav_coverage_bt_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(opennav_coverage_bt_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(opennav_coverage_bt_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND opennav_coverage_bt_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
